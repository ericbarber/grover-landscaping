...
| --- | --- | --- |
| `prototypes/minimalist-personas/` | Design direction | Ten personas have distinct task-first hierarchy, destinations, primary actions, short queues, explicit omissions, and attention/on-track/no-work scenarios. Customer, field, and operations families now add connected destination journeys, required illustrative choices, exact-plan continuity, authority separation, and bounded recovery. Actions are non-persistent and do not add roles or product authority. |
