...
| --- | --- | --- |
| `prototypes/minimalist-personas/` | Design direction | Ten personas have distinct task-first hierarchy, destinations, primary actions, short queues, explicit omissions, and attention/on-track/no-work scenarios. Yard Owner and Property Manager now add connected, destination-specific customer journeys and required illustrative choices. Actions are non-persistent and do not add roles or product authority. |
