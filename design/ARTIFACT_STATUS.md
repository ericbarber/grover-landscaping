...
| --- | --- | --- |
| `prototypes/minimalist-personas/` | Design direction | Ten personas have distinct task-first hierarchy, destinations, primary actions, short queues, explicit omissions, and attention/on-track/no-work scenarios. Customer, field, operations, administrative, and no-role experiences now add connected destination journeys, required illustrative choices, exact-plan continuity, authority separation, bounded recovery, and protected-data-safe fallback. Actions are non-persistent and do not add roles or product authority. |
