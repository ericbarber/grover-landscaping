...
| --- | --- | --- |
| `prototypes/minimalist-personas/` | Design direction | Ten personas have distinct task-first hierarchy, destinations, primary actions, short queues, explicit omissions, and attention/on-track/no-work scenarios. Customer and field families now add connected, destination-specific journeys, required illustrative choices, field authority separation, and device-safe recovery. Actions are non-persistent and do not add roles or product authority. |
