...
│   ├── validate-minimalist-personas.mjs # Task-first persona experience checks
...
Validate the separate task-first minimalist experiences for all ten personas,
three review scenarios, every destination, and the responsive reference
viewports with:
...
2. Start the active post-MVP redesign with the
   ...
   then use the
   [yard-care completion timeline](review/yard-care-completion-event-timeline.md)
   to review all expected, conditional, and gated events; use the
   [SX4 comparative study](review/simplified-product-experience-comparative-study.md)
   for matched current/new tasks; and use the
