...
Yard Owner, Property Manager, Company Owner, Company Manager, and Crew Lead,
opened the manager's Schedule path, inspected the public and provider entry
source, and traced the modern website
...
| P1 · observed in local review | **Home can reassure users when its destination cannot.** Yard Owner Home says `0 of 0` visits and “You’re clear for now,” while My yard says portal access is inactive. Crew Lead Home says three assigned stops and recommends today's Route; Route opens a past, read-only plan with two stops. See the [current journey trace](../CURRENT_JOURNEYS.md) and [Home summary logic](../../frontend/src/components/WorkspaceHomePanel.tsx). | A confident Home summary can contradict the protected access or service-day truth users see one tap later. The sample fixture also prevents a fair current-versus-new task comparison. | Make Home states use the same authorized access and date source as the destination, or withhold a confident count/action when that state is unknown. Prepare internally consistent synthetic fixtures before comparative sessions. |
