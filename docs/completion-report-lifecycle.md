...
- Persisted review-start, change-request, resubmit, and delivery transitions write audit events so the manager approval path is traceable.
- Delivery specifically writes `report_delivered` because it exposes completion evidence to the customer portal.
