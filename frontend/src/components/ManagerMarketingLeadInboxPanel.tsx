...
export function shouldShowEmptyMarketingLeadInbox(
  isLoading: boolean,
  hasError: boolean,
  visibleCount: number,
): boolean {
  return !isLoading && !hasError && visibleCount === 0;
}
...
export function ManagerMarketingLeadInboxPanel() {
  ...
  const isOverdue = (lead: MarketingLeadInboxItem) => lead.status !== 'closed'
    && Boolean(lead.next_action_at) && new Date(lead.next_action_at as string).getTime() < Date.now();
  const visible = useMemo(() => leads.filter((lead) => statusFilter === 'all'
    || (statusFilter === 'active' ? lead.status !== 'closed' : lead.status === statusFilter))
    .sort((left, right) => Number(isOverdue(right)) - Number(isOverdue(left))), [leads, statusFilter]);
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6" id="marketing-lead-inbox">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div><p className="text-xs font-black uppercase tracking-[0.16em] text-emerald-700">Platform operations</p><h2 className="mt-1 text-2xl font-black">Marketing lead inbox</h2><p className="mt-1 text-sm text-slate-500">Support-admin access only · {error ? 'Lead totals unavailable' : `${leads.filter((lead) => lead.status === 'new').length} new · ${leads.filter(isOverdue).length} overdue`}</p></div>
      ...
      {shouldShowEmptyMarketingLeadInbox(isLoading, Boolean(error), visible.length) ? <p className="mt-6 rounded-xl bg-slate-50 p-5 text-sm text-slate-600">No leads match this view.</p> : null}
      {visible.length > 0 ? <div className="mt-5 grid gap-4 lg:grid-cols-[0.8fr_1.2fr]">
        <div className="max-h-[34rem] space-y-2 overflow-y-auto pr-1">{visible.map((lead) => <button className={`w-full rounded-xl border p-4 text-left ${selected?.id === lead.id ? 'border-emerald-700 bg-emerald-50' : isOverdue(lead) ? 'border-rose-300 bg-rose-50' : 'border-slate-200'}`} key={lead.id} onClick={() => setSelectedId(lead.id)} type="button"><span className="flex justify-between gap-2"><span className="font-black">{lead.full_name}</span><span className={`text-xs font-black uppercase ${isOverdue(lead) ? 'text-rose-700' : 'text-emerald-700'}`}>{isOverdue(lead) ? 'Overdue' : lead.status}</span></span><span className="mt-1 block text-sm text-slate-600">{lead.company_name || lead.email}</span><span className="mt-2 block text-xs font-bold text-slate-400">{lead.persona.split('_').join(' ')} · {new Date(lead.created_at).toLocaleString()}</span></button>)}</div>
