...
| Yard Owner portal | [Working portal](../design/prototypes/yard-owner-portal/README.md) and [V2 handoff](../design/review/yard-owner-portal-v2-handoff.md) | Partial · persisted Home/Visits adopted | Home, Visits, Proof, and Account plus property context and delivered proof are adopted. Home/Visits use the minimized authorized persisted read with explicit recovery and no illustrative fallback. Mobilization/work-release and customer-status persistence are delivered; provider APIs and the minimized customer projection precede UI service-day states, concern recovery, recommendation collaboration, and preferences. Billing remains product-gated. |
...
5. **Active:** property-manager command-center core is adopted. The hybrid Yard
   ...
   its fail-closed Yard Owner Home/Visits adoption are delivered; continue with
   the bounded customer-safe service-day extension.
...
also delivered. The customer-safe service-day lifecycle extension is the active
repository-owned slice. Its explicit mobilization/work-release source decision
and persistence are delivered; provider write APIs and the minimized customer
projection are next. Repository maintenance and regression remain ongoing.
