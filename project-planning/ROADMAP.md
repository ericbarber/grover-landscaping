...
service-day extension. Safe Yard Crew operating preparation is adopted through
service and language facts; public fit/entry and provider identity/readiness
core are adopted with distinct owner-operator,
...
the customer-safe service-day lifecycle extension the active next slice.
...
Authenticated recipient binding now admits one verified-mailbox account and
fails closed on cross-account conflict. Phase 3B2a now lists only that actor’s
...
Aggregate SLA monitoring and the operations runbook are now delivered. Metrics
contain counts and oldest age only, distinguish unavailable from zero, and
exclude claim, organization, recipient, owner, property, and evidence labels.
Phase 3B2 is complete; bounded opportunity-response capability is next.
...
path.
Phase 4A2c4 now adds that provider workspace with remote/on-site start,
...
**In progress — Customer and provider adoption**
   ...
   - Yard Owner Home/Visits now use the explicit minimized persisted read; extend
     it through bounded customer-safe service-day states next.
