...
- The [all-persona minimal rollout](design/prototypes/yard-owner-minimal-rollout/README.md)
  ...
  Proof, and question/recommendation controls. Company Owner/Manager, Property
...
- The customer refinement slice replaces generic secondary content with exact
  Yard Owner Visits/Proof and Property Manager Properties/Proof/Approvals
  compositions. Required preparation, access, proof-feedback, and versioned-
...
- The operations refinement slice gives Company Owner, Company Manager, and
  Dispatcher separate connected decision paths. Owner business readiness leads
  into Team, Operations, and Customers; Manager service risk continues through
...
5. Frontend truth and recovery: delivered. Crew routes now derive Past,
   ...
   unavailable states withhold stale facts and provide retry/access review plus
...
- Phase 4C4a now defines the first-visit contract: an authorized provider actor
  proposes an immutable, bounded arrival window only after activation; the
  owner confirms that exact version or requests a change. The customer-facing
  lifecycle does not create a job, day plan, route stop, work order, payment,
  recurring schedule, or crew assignment and excludes provider-private
  operational data.
...
- Phase 4C4c now exposes verified-provider status/propose and property-owner
...
- Phase 6A24 makes organization profile and onboarding state fail closed.
  Profile reads, first-owner setup progress, and profile updates now return
  their existing unavailable `503` errors without PostgreSQL instead of a local
